...
class TileBackground:
    ...
    def __init__(self):
        ...
        self.w = 800 * 3
        self.h = 600 * 3
        self.tiles = [[load_image('cube%d%d.png' % (x, y))
    ...
    def draw(self):
        ...
        for ty in range(tile_bottom, tile_top+1):
            for tx in range(tile_left, tile_right+1):
                self.tiles[ty][tx].draw_to_origin(-left_offset + (
                    tx-tile_left)*800, -bottom_offset+(ty-tile_bottom)*600)
...
class InfiniteBackground:
    ...
    def draw(self):
        self.image.clip_draw_to_origin(
            self.q3l, self.q3b, self.q3w, self.q3h, 0, 0)                        # quadrant 3
        self.image.clip_draw_to_origin(
            self.q2l, self.q2b, self.q2w, self.q2h, 0, 0)                 # quadrant 2
        self.image.clip_draw_to_origin(
            self.q4l, self.q4b, self.q4w, self.q4h, 0, 0)                 # quadrant 4
        self.image.clip_draw_to_origin(
            self.q1l, self.q1b, self.q1w, self.q1h, 0, 0)          # quadrant 1
