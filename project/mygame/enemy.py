...
class Enemy1:
    ...
    def find_player(self):
        ...
        print((PIXEL_PER_METER * 10) ** 1.5)
        if distance < float((PIXEL_PER_METER * 10) ** 1.5):
            return BehaviorTree.SUCCESS
        else:
            self.speed = 0
    ...
    def attack_to_player(self):
        ...
        if self.attack_count == 0:
            enemy_shot = Bullet(server.hero.x, server.hero.y, self.x, self.y, 1)
            game_world.add_object(enemy_shot, 1)
            self.attack_count += 1
            if enemy_shot.x <= 100 or enemy_shot.x >= 1900 or enemy_shot.y <= 100 or enemy_shot.y >= 1100:
                game_world.remove_object(enemy_shot)
        if self.frame >= 10.9:
            self.attack_count = 0
    ...
    def prepare_patrol_points(self):
        ...
        positions = [(random.randint(1400, 3000), random.randint(100, 1200)), (1118, 750), (1050, 530), (575, 220), (235, 33), (575, 220), (1050, 530), (1118, 750)]
    ...
    def draw(self):
        if self.attack == False:
            ...
            self.image_attack.clip_draw(int(self.frame) * 45, 67, 45, 67, self.x, self.y, enemy_size, enemy_size)
    ...
    def get_bb(self):
        if self.attack == True:
            ...
            return self.x - 5, self.y - 30, self.x + 80, self.y + 30
